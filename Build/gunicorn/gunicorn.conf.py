...
def post_fork(server, worker):
    """
    Output logging to say that we've spawned a worker
    """
    server.log.info("Worker spawned (pid: %s)", worker.pid)

def pre_fork(server, worker):
    """
    Use this function to perform tasks prior to a reload
    """
    pass

def pre_exec(server):
    """
    Output logging to say we are reloading
    """
    server.log.info("Forked child, re-executing.")

def when_ready(server):
    """
    Output logging info to state the server is ready
    """
    server.log.info("Server is ready. Spawning workers")

def worker_int(worker):
    """
    Tell the worker to quit or interrupt
    """
...
def worker_abort(worker):
    """
    Function to tell the worker to abort it's process
    """
